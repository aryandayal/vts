...
import { useGoods, useGodowns, useUsers, useProfile, useClearData } from "./hooks/useData"; // Import the specific hooks
...
function App() {
  ...
  const { fetchGodowns, fetchDeletedGodowns } = useGodowns();
  ...
  // Initialize the data for the app when authenticated
  useEffect(() => {
    if (isAuthenticated && storeReady) {
      // Fetch all necessary data when the app loads and user is authenticated
      fetchGoods();
      fetchGodowns();
      fetchDeletedGodowns();
      fetchUsers();
  ...
  }, [isAuthenticated, storeReady, fetchGoods, fetchGodowns, fetchDeletedGodowns, fetchUsers, fetchProfile]);
  ...
  }, [isAuthenticated, authChecked, clearData]);
