...
// Helper functions (keep those that are UI-specific)
const calculateBounds = (coordinates) => {
  if (!coordinates || coordinates.length === 0) return null;
...
const VehicleTableRow = React.memo(({ 
  device, 
  imei, 
  index, 
  isSelected, 
  ...
  return (
    <tr 
      className={isSelected ? styles.trSelectedRow : ''} 
      onClick={handleRowClick}
    >
      <td onClick={handleCheckboxClick}>
        <input 
          type="checkbox" 
          onChange={() => onVehicleSelect(imei)}
          onClick={(e) => e.stopPropagation()}
        />
      </td>
      <td>{index + 1}</td>
...
// Add a skeleton row component
const SkeletonRow = () => (
  <tr>
    {[...Array(9)].map((_, i) => (
      <td key={i}>
        <div className={styles.skeleton} style={{ height: '16px', width: i === 0 ? '20px' : '100%' }}></div>
      </td>
    ))}
  </tr>
);
...
const VehicleTable = ({ devices, onDeviceSelect, selectedDeviceId, loading, error }) => {
  ...
  useEffect(() => {
    if (!loading && devices && Object.keys(devices).length > 0) {
  ...
  // Memoize the device entries to avoid unnecessary recalculations
      ...
      {deviceEntries.length === 0 ? (
        ...
        <div className={styles.tableWrapper}>
          <table className={styles.vehicleTable}>
            ...
            <tbody>
              {deviceEntries.map(([imei, device], idx) => (
                <VehicleTableRow
                  key={imei}
                  device={device}
                  imei={imei}
...
function MapView() {
  ...
  // Initialize GPS data on component mount
  useEffect(() => {
    const initialize = async () => {
      try {
        const cleanup = await initializeData();
        return cleanup;
      } catch (error) {
    ...
    initialize().then(cleanup => {
      ...
      const cleanupRef = { current: cleanup };
  ...
  }, [devicesData, realtimeTracks, setSelectedDeviceId]);
